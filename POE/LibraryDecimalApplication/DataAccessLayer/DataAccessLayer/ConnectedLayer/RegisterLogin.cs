    ...
    public class RegisterLogin
     {
        ...
        public void InsertUserRegData(ModelRegisterLogin user, string cs)
        {
            ...
            {
                var cmd = cnn.Query<ModelRegisterLogin>("Insert Into USER(USER_STUDENT_CARD_ID, USER_PASSWORD,USER_FNAME,USER_LNAME) Values" + $"('{user.USER_STUDENT_CARD_ID}','{user.USER_PASSWORD}','{user.USER_FNAME}','{user.USER_LNAME}')", new DynamicParameters());
            }
        ...
        public List<ModelRegisterLogin> GetUser(int id,string cs)
        {
            using (IDbConnection cnn = new SQLiteConnection(cs))
            {
                var list = cnn.Query<ModelRegisterLogin>("SELECT * FROM USER WHERE USER_STUDENT_CARD_ID= '" + id + "'", new DynamicParameters());
                return list.ToList();
            }
        }
